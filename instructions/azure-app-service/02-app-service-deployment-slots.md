...
この演習では、静的 HTML Web サイトを Azure App Service にデプロイし、ステージング デプロイ スロットを作成し、コードに変更を加えてステージング スロットにデプロイし、ステージング スロットと運用スロットを交換して変更を運用環境に昇格させます。 安全なアプリケーション更新とブルーグリーン デプロイのためにデプロイ スロットを使用する方法について説明します。
...
1. 次のコマンドを実行し、リソース グループとアプリ名を保持する変数を設定します。 使用したいリソース グループがある場合は、**rg-mywebapp** の値を **resourceGroup** に置き換えることができます。 コマンド実行後に表示される **appName** の値をメモしておきます。この演習の後半で必要になります。
...
1. サンプル コードが含まれているディレクトリに移動し、**[az webapp up]** コマンドを実行します。 **注:** このコマンドの実行には数分かかることがあります。
    ...
    >**注:**  **az webapp up** コマンドが機能するようにするには、Cloud Shell で **az login** または **az login --use-device-code** を実行することが必要な場合があります。
...
1. Cloud Shell で「**code index.html**」と入力してエディターを開きます。 **\<h1\>** 見出しタグを見つけて、*Azure App Service - Sample Static HTML Site* を *Azure App Service Staging Slot*、または他の任意の名前に変更します。
...
Azure portal では、ツール バーの **[Swap]** オプションを使用してスワップを実行できます。 Web アプリの左側のメニューで **[Overview]** または **[Deployment] > [Deployment slots]** を選択すると、ツール バーに **[Swap]** オプションが表示されます。
